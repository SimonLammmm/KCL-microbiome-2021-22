...
c("A", "B", "C") == LETTERS # the LHS and RHS objects are not exactly the same, so FALSE is returned once
...
c("Chemical carcinogenesis", "Chronic inflammation") %in% my_plaintext$Term # TRUE FALSE
...
if(my_pvalue2 < 0.001) {         # condition 1: less than 0.001
    cat("***\n")                 # print ***
} else if(my_pvalue2 < 0.01) {   # condition 2: less than 0.01
    cat("**\n")                  # print **
} else if(my_pvalue2 < 0.05) {   # condition 3: less than 0.05
    cat("*\n")                   # print *
} else if(my_pvalue2 >= 0.05) {  # condition 4: greater than or equal to 0.05
    cat("n.s.\n")                # print n.s.
}
...
We can iterate over a list using the `for()` loop.
...
for (geneset in unique(my_newdata$Gene_set)) {
    ...
    cat("The sum for", geneset, "is", my_sum, "\n")
}
